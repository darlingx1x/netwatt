...
const DrawerButton = forwardRef<HTMLButtonElement, DrawerButtonProps>(
...
export function DrawerBackdrop({
  ...
  return (
    <DrawerPrimitive.Backdrop
      className={cn(
        // NOTE: previously had `supports-[-webkit-touch-callout:none]:absolute`
        // — that flipped the backdrop to position:absolute on iOS so it
        // anchored to the document, not the viewport. When the user scrolled
        // the page down before opening the drawer, the backdrop's top edge
        // stayed at document y=0, leaving an unblurred strip below the
        // Telegram chrome. Stick with position:fixed everywhere so the
        // backdrop always covers the visible viewport.
        "fixed inset-0 z-50 bg-black/40 opacity-[calc(1-var(--drawer-swipe-progress))] backdrop-blur-md transition-opacity duration-450 ease-[cubic-bezier(0.32,0.72,0,1)] data-ending-style:opacity-0 data-starting-style:opacity-0 data-ending-style:duration-[calc(var(--drawer-swipe-strength)*400ms)] data-swiping:duration-0",
        className,
      )}
        ...
        >
          ...
          {showCloseButton && (
            <DrawerPrimitive.Close
              ...
              render={<DrawerButton size="icon" variant="ghost" />}
            >
              <XIcon />
            </DrawerPrimitive.Close>
          )}
...
export { DrawerPrimitive, DrawerButton };
